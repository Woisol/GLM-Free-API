...
- 流式缓冲曾存在卡死风险：把未完成的 `【turn0` 当作待发送文本的边界后，`sentContent` 可能永远无法与后续清理后的快照对齐。现改为先对每个累计快照生成安全文本，再仅发送安全文本相对已发送前缀的后缀；未完成标识不会进入发送前缀，也不会阻塞后续正文。
- 真实搜索回归还发现上游标识类型不只 `search`，还包括 `fetch`、`fetch0` 等形式，例如 `【turn1fetch0】`，并且可能拆成 `【turn1fetch` 与后续数字。原先只匹配 `search` 时会先发送未清理前缀，随后累计快照无法匹配已发送前缀而停止输出。
- 现统一匹配 `【turn数字+字母类型+数字】`，并把 `reasoning_content` 与正文使用相同的清理逻辑。
- 使用真实搜索请求回归通过：包含 `turn1fetch0` 等标识的长流正常完成并发送 `[DONE]`，客户端响应中无残留标识。
